...
namespace MobileManager.GitClient.Utils
{
    /// <summary>
    /// External processes.
    /// </summary>
    public class ExternalProcesses : IExternalProcesses
    {
        ...
        public string RunProcessAndReadOutput(string processName, string processArgs)
        {
            var proc = $"{processName} {processArgs}";
            var response = Shell.Term(proc, Output.Hidden);
